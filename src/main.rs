...
use api::{
    interfaces::t_youtube_service::TYouTubeService, oauth2_service::{self, OAuth2Service}, youtube_repo::YouTubeRepository, youtube_service::YouTubeService
};
use dotenv::dotenv;
...
async fn main() -> anyhow::Result<(), Box<dyn std::error::Error>> {
    ...
    let result = api.subscribe(&api_key, &channels, &mut secrets).await?;
